        ...
        protected BinarySerializable(SerializationInfo info, StreamingContext context)
        {
            ...
            foreach (FieldInfo field in T.GetFields())
            {
                ...
                if (serializedType.IsArray)
                {
                    ...
                    Array deserializedValueA = Array.CreateInstance(serializedElementType, nItems);
                    for (int i = 0; i < nItems; i++)
                        deserializedValueA.SetValue(info.GetValue($"{field.Name}__{i}", serializedElementType), i);
                    deserializedValue = deserializedValueA;

                }
                else
                {
                    deserializedValue = info.GetValue(field.Name, field.FieldType);
                }
                object value;
