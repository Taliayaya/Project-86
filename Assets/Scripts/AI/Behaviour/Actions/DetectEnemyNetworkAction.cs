...
[NodeDescription(name: "Detect Enemy Network", story: "Find visibleTarget close to [Agent] or from LegionNetwork and mark them", category: "Action", id: "2a71e47fe609fbbd2407d837180c3ff6")]
public partial class DetectEnemyNetworkAction : Action
{
    ...
    [SerializeReference] public BlackboardVariable<int> spotDurationToBeDetected = new(5);
    ...
    private IEnumerator Detect()
    {
        while (true)
        {
            ...
            // somehow yield return DetectTargets() doesnt work
            IEnumerator detectTargets = DetectTargets();
            while (detectTargets.MoveNext()) yield return null;
            
            // if we dont find any target nearby, go ask the network
            if (_nextVisibleTargets.Count == 0)
                DetectNetworkTargets();
    ...
    private void DetectNetworkTargets()
    {
        foreach (SharedTarget sharedTarget in LegionNetwork.Instance.NetworkTargets)
        {
            var copy = sharedTarget;
            if (sharedTarget.SpotDuration < spotDurationToBeDetected.Value ) continue;
            if (!copy.Target.TryGet(out var targetObj) || !targetObj) 
                continue;

            float distance = Vector3.Distance(Agent.Value.transform.position, copy.SpotPosition);
            if (distance > AgentSo.Value.combatDistance)
                continue;
            
            Debug.Log($"{Agent.Value.name} network detected {targetObj.name} ({sharedTarget.SpotDuration}");
            _nextVisibleTargets.Add(targetObj.GetComponent<Unit>());
        }
    }
